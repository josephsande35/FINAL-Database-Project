...
-- Sub-subclasses (Removed redundant email column)
...
CREATE TABLE Blood_Unit (
    ...
    Volume DECIMAL(6,2) NOT NULL CHECK (Volume > 0),
    Status VARCHAR(20) DEFAULT 'Collected' CHECK (Status IN ('Collected','Screening','Approved','Rejected','Used'))
);
...
-- =============================================
-- STEP 3: ROLE & USER CONFIGURATION
-- =============================================

-- Create Group Roles (NOLOGIN ensures they are just groups)
CREATE ROLE donor_role NOLOGIN;
CREATE ROLE field_staff_role NOLOGIN;
CREATE ROLE drive_staff_role NOLOGIN;
CREATE ROLE admin_role NOLOGIN;

-- Create Specific Users
CREATE USER donor_user WITH PASSWORD 'donor123';
CREATE USER fieldstaff_user WITH PASSWORD 'field123';
CREATE USER drivestaff_user WITH PASSWORD 'drive123';
CREATE USER admin_user WITH PASSWORD 'admin123';

-- Grant Group Membership
GRANT donor_role TO donor_user;
GRANT field_staff_role TO fieldstaff_user;
GRANT drive_staff_role TO drivestaff_user;
...
CREATE OR REPLACE VIEW donor_view AS
SELECT 
    d.Donor_ID, 
    p.First_name || ' ' || p.Last_name as Name, 
    d.Blood_Type, 
    d.Date_Last_Donation,
    a.Status, 
    de.Location, 
    de.Date as Event_Date
FROM Donor d 
JOIN Person p ON d.Person_ID = p.Person_ID 
LEFT JOIN Appointment a ON d.Donor_ID = a.Donor_ID 
LEFT JOIN Drive_Event de ON a.Event_ID = de.Event_ID;

CREATE OR REPLACE VIEW drive_staff_view AS
SELECT 
    de.Event_ID,
    de.Location,
    de.Date,
...
FROM Drive_Event de 
LEFT JOIN Appointment a ON de.Event_ID = a.Event_ID 
GROUP BY de.Event_ID, de.Location, de.Date;
...
GRANT SELECT ON drive_staff_view TO drive_staff_role; 
GRANT SELECT, INSERT, UPDATE ON Drive_Event, Appointment TO drive_staff_role;

GRANT ALL ON ALL TABLES IN SCHEMA public TO admin_role;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO admin_role;

-- =============================================
-- STEP 6: PROCEDURES & FUNCTIONS
-- =============================================

-- Procedure: Donor eligibility check
CREATE OR REPLACE PROCEDURE check_donor_eligibility(p_donor_id INT),
LANGUAGE plpgsql AS $$
DECLARE 
    v_last_donation DATE;
BEGIN
    SELECT Date_Last_Donation INTO v_last_donation 
    FROM Donor 
    WHERE Donor_ID = p_donor_id;

    IF v_last_donation IS NULL OR (CURRENT_DATE - v_last_donation) >= 112 THEN
        RAISE NOTICE 'Donor % is eligible.', p_donor_id;
    ELSE 
        RAISE EXCEPTION 'Donor not eligible. Wait until %', v_last_donation + INTERVAL '112 days';
    END IF;
END;
$$;
...
BEGIN
    UPDATE Donor 
    SET Date_Last_Donation = CURRENT_DATE 
...
--backup strategies---
mysqldump -u admin_user -p blood_donation_db --single-transaction --routines --triggers > /path/to/backup/full_backup.sql
mysqldump -u admin_user -p mysql user > /path/to/backup/mysql_users_$(date +%Y%m%d).sql
[mysqld]
server-id = 1
log_bin = /var/log/mysql/mysql-bin
binlog_format = ROW
expire_logs_days = 7  # Automatically purge logs older than 7 days